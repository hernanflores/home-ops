...
export async function readTracker(path, validate) {
...
export async function writeTracker(path, records, expectedSource = null) {
  await mkdir(dirname(path), { recursive: true });
  if (expectedSource !== null && await content(path) !== expectedSource) throw new Error(`Tracker changed during update: ${path}`);
...
export async function withTrackerLock(path, callback, timeoutMs = 5000) {
  await mkdir(dirname(path), { recursive: true });
  ...
  while (!handle) {
    try {
      ...
      if (await lockIsStale(lockPath, timeoutMs)) {
        const reclaimed = await reclaimStaleLock(lockPath, timeoutMs, deadline);
        if (!reclaimed) throw new Error(`Tracker stale-lock recovery is held by another process: ${path}`);
        continue;
      }
      if (Date.now() >= deadline) throw new Error(`Tracker is locked by another update: ${path}`);
      await delay(10);
    }
  }
  try {
